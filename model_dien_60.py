...
def get_history_din_embedded(self):
    ...
    for fir in his_days[::-1]:
        ...
        get_rnn_sum(getattr(self, key_c),"rnncha"))
        history_sec_embedded.append(
        get_rnn_sum(getattr(self, key_s),"rnnsec" ))
    ...
    with tf.name_scope('rnn_1'):
        rnn_outputs, _ = dynamic_rnn(GRUCell(HIDDEN_SIZE*2), inputs=tf.concat([history_cha_emb,history_sec_emb],axis=-1),
                                        sequence_length=self.history_len_ph, dtype=tf.float32,
                                        scope="gru1")
    with tf.name_scope('Attention_layer_1'):
        att_outputs, alphas = din_fcn_attention(tf.concat([get_rnn_sum(self.today_chapters_embedded,"rnncha"),
                   get_rnn_sum(self.today_sections_embedded,"rnnsec")],axis=-1),
         rnn_outputs, ATTENTION_SIZE, self.history_mask_cha_ph,
                                            scope="1_1",softmax_stag=1, stag='1_1', mode='LIST', return_alphas=True)
    with tf.name_scope('rnn_2'):
        rnn_outputs2, final_state2 = dynamic_rnn(VecAttGRUCell(HIDDEN_SIZE*2), inputs=rnn_outputs,
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('Main_Embedding_layer'):
                ...
                with tf.name_scope("Homework"):
                    ...
                    self.history_chap_embedded= get_history_din_embedded(self)
...
class SimpleModel(Model):
    def __init__(self, *, use_dice=False):
        ...
        homework.append(self.history_chap_embedded *tf.concat([self.today_cha_rnn,self.today_sec_rnn],axis=-1))
