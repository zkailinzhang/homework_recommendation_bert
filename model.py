...
def get_history_sum_embedded(self):
    ...
    self.history_one_chap_embedded = tf.nn.embedding_lookup(self.chapters_embeddings_var,
                                                            self.history_one_chap_ph)
    self.history_two_chap_embedded = tf.nn.embedding_lookup(self.chapters_embeddings_var,
                                                            self.history_two_chap_ph)
    self.history_three_chap_embedded = tf.nn.embedding_lookup(self.chapters_embeddings_var,
                                                              self.history_three_chap_ph)
    self.history_one_sec_embedded = tf.nn.embedding_lookup(self.sections_embeddings_var,
                                                           self.history_one_sec_ph)
    self.history_two_sec_embedded = tf.nn.embedding_lookup(self.sections_embeddings_var,
                                                           self.history_two_sec_ph)
    self.history_three_sec_embedded = tf.nn.embedding_lookup(self.sections_embeddings_var,
                                                             self.history_three_sec_ph)
    
    #
    chap = tf.reduce_sum(self.history_one_chap_embedded, axis=-2)
    for c in [self.history_two_chap_embedded,
              self.history_three_chap_embedded, ]:
        chap = tf.concat([chap, tf.reduce_sum(c, axis=-2)], axis=-1)

    sec = tf.reduce_sum(self.history_one_sec_embedded, axis=-2)
    for c in [self.history_two_sec_embedded,
              self.history_three_sec_embedded, ]:
        sec = tf.concat([sec, tf.reduce_sum(c, axis=-2)], axis=-1)
    ...
    def __init__(self, *, use_dice=False):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('Main_Inputs'):
                ...
                #所以N N 
                self.history_one_chap_ph = tf.placeholder(tf.int32, [None, None], name="history_one_chap_ph")
                self.history_two_chap_ph = tf.placeholder(tf.int32, [None, None], name="history_two_chap_ph")
                self.history_three_chap_ph = tf.placeholder(tf.int32, [None, None], name="history_three_chap_ph")
                self.history_one_sec_ph = tf.placeholder(tf.int32, [None, None], name="history_one_sec_ph")
                self.history_two_sec_ph = tf.placeholder(tf.int32, [None, None], name="history_two_sec_ph")
                self.history_three_sec_ph = tf.placeholder(tf.int32, [None, None], name="history_three_sec_ph")
                ...
                self.study_vector_ph = tf.placeholder(tf.int32, [None, 20], name="study_vector_ph")
            ...
            with tf.name_scope('Main_Embedding_layer'):
                ...
                with tf.name_scope("Homework"):
                    ...
                    self.today_chapters_embedded = tf.nn.embedding_lookup(self.chapters_embeddings_var,
                                                                          self.today_chapters_ph)
                    self.today_sections_embedded = tf.nn.embedding_lookup(self.sections_embeddings_var,
                                                                          self.today_sections_ph)
                    self.history_sum_embedded = get_history_sum_embedded(self)
...
class SimpleModel(Model):
    def __init__(self, *, use_dice=False):
        ...
        homework.append(tf.reduce_sum(self.today_chapters_embedded, -2))
        homework.append(tf.reduce_sum(self.today_sections_embedded, -2))
        homework.append(self.history_sum_embedded)

        reflect = []
        reflect.append(tf.reduce_sum(self.reflect_value_embedded, -2))
