...
def teacher_regis_time (cursor, teacher_id,curentday):
    
    sql = """SELECT register_day FROM r_teacher_info WHERE teacher_id = {}"""

    cursor.execute(sql.format(teacher_id))
    res = cursor.fetchall()
    base_ = res[0] if len(res) > 0 else {"0000-00-00"}
    base = base_["register_day"].strftime("%Y-%m-%d")
    gapday=0
    curday = datetime.datetime.strptime(curentday,"%Y-%m-%d")
    regisday = datetime.datetime.strptime(base,"%Y-%m-%d")
    gapday = (curday - regisday).days
...
if __name__ == "__main__":
    ...
    with tf.Session(graph=mol.graph) as sess:
                ...
                with feature.get_coursor("recom", "dict") as ff:
                    ...
                    for item in data_gb:
                        ...
                        for i in range(HIS_DAYS_NUM):
                            his_info = feature.yesterday(ff, class_id, day, i+1)
                            his_cha.append(list(his_info.values())[0])

                        max_len = find_LCIS(his_cha)
                        gapday =0
                        if max_len ==14:
                            gapday =teacher_regis_time(ff,teacher_id,curentday)
                            result.append(gapday)
  
                        
                        result2.append(max_len)
        ...
        print(result)  
        with open(os.path.join(PATH,"result_data6.txt"),'a') as f:
            f.write(str(result))   
        print(result2)
        with open(os.path.join(PATH,"result_data7.txt"),'a') as f:
            f.write(str(result2))  
