    ...
    with tf.name_scope("GRU"):
        ...
        with tf.name_scope('gru1'):
            def get_cell():
                ...
                cell2_ = tf.nn.rnn_cell.DropoutWrapper(cell2, output_keep_prob=KEEP_PROB)
                return cell2_
            cells = [get_cell() for _ in range(num_layers)]
            Cell = tf.nn.rnn_cell.MultiRNNCell(cells, state_is_tuple=True)
            rnnoutput, _ = tf.nn.dynamic_rnn(cell=Cell, inputs=history_cha_emb, dtype=tf.float32,scope='cha')
    ...
    with tf.name_scope("GRU2"):
        ...
        with tf.name_scope('gru2'):
            ...
            for _ in range(num_layers):
                ...
                cell_ = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=KEEP_PROB)
                cells2.append(cell_)
            #cells2 = [get_cell2() for _ in range(num_layers)]
            Cell2 = tf.nn.rnn_cell.MultiRNNCell(cells2, state_is_tuple=True)
            rnnoutput2, _ = tf.nn.dynamic_rnn(cell=Cell2, inputs=history_sec_emb, dtype=tf.float32,scope='SEC')
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('Main_Embedding_layer'):
                ...
                with tf.name_scope("Homework"):
                    ...
                    self.history_chap_embedded,self.history_sec_embedded  = get_history_gru_embedded(self)
