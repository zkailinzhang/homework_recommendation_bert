...
from utils import tf, prelu, dice,din_attention,din_fcn_attention,QAAttGRUCell,VecAttGRUCell
import numpy as np
...
def get_history_din_embedded(self):
    ...
    with tf.name_scope('rnn_1'):
        rnn_outputs, _ = dynamic_rnn(GRUCell(HIDDEN_SIZE), inputs=history_cha_emb,
                                        sequence_length=self.history_len_ph, dtype=tf.float32,
                                        scope="gru1")
    with tf.name_scope('Attention_layer_1'):
        att_outputs, alphas = din_fcn_attention(tf.reduce_mean(self.today_chapters_embedded,axis=-2),
         rnn_outputs, ATTENTION_SIZE, self.history_mask_cha_ph,
                                            scope="1_1",softmax_stag=1, stag='1_1', mode='LIST', return_alphas=True)
    ...
    with tf.name_scope('rnn_11'):
        rnn_outputs3, _ = dynamic_rnn(GRUCell(HIDDEN_SIZE), inputs=history_sec_emb,
                                        sequence_length=self.history_len_ph, dtype=tf.float32,
                                        scope="gru11")
    with tf.name_scope('Attention_layer_11'):
        att_outputs3, alphas3 = din_fcn_attention(tf.reduce_mean(self.today_sections_embedded,axis=-2),
         rnn_outputs3, ATTENTION_SIZE, self.history_mask_sec_ph,
                                                scope="2_2",softmax_stag=1, stag='2_2', mode='LIST', return_alphas=True)
    with tf.name_scope('rnn_22'):
        rnn_outputs4, final_state3 = dynamic_rnn(VecAttGRUCell(HIDDEN_SIZE), inputs=rnn_outputs3,
    ...
    def __init__(self, *, use_dice=False):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('Main_Inputs'):
                ...
                #self.history_mask_ph = tf.placeholder(tf.float32, [None, None], name="history_mask_ph")
                self.history_len_ph = tf.placeholder(tf.float32, [None], name="history_len_ph")
                self.history_mask_cha_ph = tf.placeholder(tf.float32, [None, None], name="history_mask_cha_ph")
                self.history_mask_sec_ph = tf.placeholder(tf.float32, [None, None], name="history_mask_sec_ph")
            ...
            with tf.name_scope('Main_Embedding_layer'):
                ...
                with tf.name_scope("Homework"):
                    ...
                    self.history_chap_embedded,self.history_sec_embedded = get_history_din_embedded(self)
