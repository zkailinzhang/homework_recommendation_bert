...
flags.DEFINE_string("logfile",'logs.dien.60.out',"log file to save")
flags.DEFINE_string("csvfile","test_metric_day.dien.60.csv",'csv file to save test metric')
...
RST_PATH = os.path.join(PATH, "result_dien_60")

BASE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "save_dien_60")
...
def prepare_data(data):
    ...
    resp["history_len_ph"]= np.ones(shape=(len(his_days))).astype("float32")
    ...
    for i in range(sample_nums):
        ...
        history_mask_cha_ = [True if cha_days_aim[i][0] else False for i  in range(HISTOYR_HOMEWOEK_DAYS)]
        history_mask_cha.append(history_mask_cha_)
        ...
        history_mask_sec_ = [True if cha_days_aim[i][0] else False for i  in range(HISTOYR_HOMEWOEK_DAYS)]
        history_mask_sec.append(history_mask_sec_)
        num=0
        for item in sec_[::-1]:
            resp[item][i] = sec_days_aim[num]
            num+=1
    resp["history_mask_cha_ph"] =   np.asarray(history_mask_cha).astype("float32")
    resp["history_mask_sec_ph"] =   np.asarray(history_mask_sec).astype("float32")
                ...
                for i in range(epoches):
                    for data_pre in train_data:
                        ...
                        if iiter % print_iter == 0:
                            ...
                            logging.info("---train--- day:{}, epoch:{}, iter: {},loss_average:{}, accuracy_average:{},loss:{},acc:{}".format(
                            curentday,i,iiter,
