...
from utils import tf, prelu, dice,din_attention
import numpy as np
...
def get_history_din_embedded(self):
    ...
    attention_cha_output = din_attention(
        tf.reduce_mean(self.today_chapters_embedded,axis=-2),
        history_cha_emb, 
        ATTENTION_SIZE, self.history_mask_cha_ph,stag="cha")

    history_sec_emb = tf.transpose(history_sec_embedded, [1, 0, 2])

    attention_sec_output = din_attention(
        tf.reduce_mean(self.today_sections_embedded,axis=-2 ),
        history_sec_emb, 
        ATTENTION_SIZE, self.history_mask_sec_ph,stag="sec")
            ...
            with tf.name_scope('Main_Embedding_layer'):
                ...
                with tf.name_scope("Style"):
                    for fir in ["1", "2", "3", "4"]:
                        for sec in ["100", "010", "001", "110", "101", "011", "111"]:
                            key = "style_" + fir + "0" + sec + "_ph"
                            embedings_var = "style_" + fir + "0" + sec + "_embeddings_var"
                            embed_key = "style_" + fir + "0" + sec + "_embedded"
                            setattr(self, embedings_var,tf.get_variable(embedings_var,
                                                                [N_STYLE_HIS, EMBEDDING_DIM]))
                            setattr(self, embed_key,
                                    tf.nn.embedding_lookup(getattr(self, embedings_var),getattr(self,key)))
                ...
                with tf.name_scope("Homework"):
                    ...
                    self.today_cha_rnn = get_rnn_sum(self.today_chapters_embedded,"tocha")
                    self.today_sec_rnn = get_rnn_sum(self.today_chapters_embedded,"tosec")
                # reflect
...
class SimpleModel(Model):
    def __init__(self, *, use_dice=False):
        ...
        homework.append()
        homework.append(self.today_cha_rnn)
        homework.append(self.today_sec_rnn)
        homework.append(self.history_sec_embedded)
        homework.append(self.history_chap_embedded)
        homework.append(self.history_chap_embedded * self.today_cha_rnn)
        homework.append(self.history_sec_embedded * self.today_sec_rnn)
