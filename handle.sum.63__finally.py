...
flags.DEFINE_string("logfile",'logs.sum.63.out',"log file to save")
flags.DEFINE_string("csvfile","test_metric_day.sum.63.csv",'csv file to save test metric')

logs_path = os.path.join(PATH,"boardlog")
...
RST_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "result_sum_63")
BASE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "save_sum_63")
...
def prepare_data(data):
    ...
    keep = ["teacher_id_ph", "student_count_ph",
            ...
            "analysis_avg_times_ph","analysis_avg_rate_ph","analysis_avg_exp_score_ph",
            "analysis_avg_exp_level_ph"
    ...
    for i in range(sample_nums):
        ...
        for item in sec_[::-1]:
            ...
            tmp = resp[item][i]
            sec_days.append(tmp)
        sec_aim = repeat(sec_days)
        
        num=0
        for item in sec_[::-1]:
            resp[item][i] = sec_aim[num]
            num+=1

    sty_his_ = list(map(lambda x: "style_" + x + "_ph", his_days))
    #
    for item in sty_his_[::-1]:    
    ...
    for i in range(sample_nums):
        ...
        for item in sty_his_[::-1]:
            tmp = resp[item][i]
            sty_days.append(tmp)
        sty_days_aim = repeat_sty(sty_days)
        
        num=0
        for item in sty_his_[::-1]:
            resp[item][i] = sty_days_aim[num]
            num+=1
...
if __name__ == "__main__":
    ...
    with tf.Session(graph=mol.graph) as sess:
        ...
        while int(''.join(curentday.split('-'))) < int(''.join(nowday.split('-'))):
            ...
            if int(''.join(curentday.split('-'))) >= int(''.join("2019-11-24".split('-'))):
